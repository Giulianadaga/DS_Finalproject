...
data_weather_survey <- read_dta("Data/weather_survey_data.dta") 
...
splits <- initial_split(data_weather_survey,prop = .75, strata = total_profit_win)
...
We can see that almost all our survey continuous variables are skewed to the left side, with the exception of av. age and fraction of females. Therefore, we will log transform them before normalizing. 
...
rcp <- 
  ...
  step_modeimpute(all_nominal()) %>% # Impute all missings in factors with the mode
  step_dummy(all_nominal()) %>% # Convert all factor variables into dummies which is better for the model
    step_log(total_profit_win, lmax_educ, lmax_educ_head, lagric_insurance, lav_age, lasset_car, lasset_irrig_equip, lasset_radio, lasset_wheelbarrow, lcurrent_acres_maize_dry, lcurrent_acres_beans, lharvest_beans_kg_win, lharvest_maize_dry_kg_win, linput_chem_insect, linput_irrigation, linput_manure, linput_no_input, lmaize_dry_kg_peracre_win, lmale_head, lnb_acres_own_current, lnb_acres_rented_lastseason, lnb_acres_farmed_lastseason, lrevenue_harvest, offset = 1) %>%
    step_range(lhead_age:lasset_car) %>%  # Normalize scale
  prep()
...
surrogate_tree <-
  ...
  rpart::rpart(
    ...
    # the RF model
    profits_pred ~ lnb_acres_rented_lastseason + lnb_acres_own_current + lnb_acres_farmed_lastseason + lmax_educ_head + lmax_educ + lmale_head + linput_irrigation + lhead_age + lfrac_women + lav_age,
